...
class XGBModelPersistor(ModelPersistor):
    ...
    def get_model_from_location(self, location, fl_ctx: FLContext) -> ModelLearnable:
        """Initialize and load the Model.
        ...
        if os.path.exists(location):
            self.logger.info(f"Loading server model From -> {location} <-") 
        ...
        self.log_info(fl_ctx, f"The persistor model is 123:{model}")
        model_learnable = make_model_learnable(weights=model, meta_props=dict())
        self.log_info(fl_ctx, f"The persistor learnable is 123:{model}")
        return model_learnable
